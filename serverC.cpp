...
const string blockFile = "block3.txt";
int lastTx = -100;
...
int startServer() {
    ...
    servaddr.sin_port = htons(serverCPort);

    // Bind the socket with the server address 
    if (bind(sockfd, (const struct sockaddr *)&servaddr,sizeof(servaddr)) < 0) { 
        perror("Server C could not bind the socket!"); 
        exit(EXIT_FAILURE); 
    }

    string portInStrFormat = "\"" + to_string(serverCPort) + "\"";
    string msg = "The Server C is up and running using UDP on port " + portInStrFormat;
    cout << msg << "\n";
    ...
    while (true) {
        ...
        if (clientPort == 24250) {
            cout << "The ServerC received a request from the Main Server.\n";
            ...
            if (requestVector[0] == "1") {
                ...
                if (bytesSent < 0) {
                    perror("The ServerC failed to send the response to the Main Server.");
                } else {
                    cout << "The ServerC finished sending the response to the Main Server.\n";
                }
            }
            // if code is 2 - send last tx number along with the usual required data as code "1"
            // serverM will make 2 requests.
            ...
            // record transaction into the log
                ...
                if (recordTransaction(requestVector)) {
                    ...
                    perror("The ServerC failed to record the transaction into the log.");
