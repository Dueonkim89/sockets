...
const string blockFile = "block1.txt";
int lastTx = -100;
...
int startServer() {
    ...
    if (bind(sockfd, (const struct sockaddr *)&servaddr,sizeof(servaddr)) < 0) { 
        perror("Server A could not bind the socket!"); 
        exit(EXIT_FAILURE); 
    }

    string portInStrFormat = "\"" + to_string(serverAPort) + "\"";
    string msg = "The Server A is up and running using UDP on port " + portInStrFormat;
    cout << msg << "\n";
    ...
    while (true) {
        ...
        if (clientPort == 24250) {
            cout << "The ServerA received a request from the Main Server.\n";
            ...
            // record transaction into the log
                ...
                if (recordTransaction(requestVector)) {
                    ...
                    perror("The ServerA failed to record the transaction into the log.");
